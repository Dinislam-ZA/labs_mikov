...
class MainView : View() {
                    ...
                    for (k in 1..iter) {
                        ...
                        try {
                            if (l == 1) {
                                ...
                                val nk: Int
                                if (k == 1) {
                                    nk = inputDialogThreadSafe("Ввод размерности", "N=").toInt()
                                    n[k - 1] = nk
                                } else {
                                    n[k - 1] = m[k - 2]
                                    logMessage("N=${n[k - 1]}")
                                ...
                                val mk = inputDialogThreadSafe("Ввод размерности", "M=").toInt()
                                m[k - 1] = mk
                            ...
                            if (k == 1) {
                                ...
                                val xValues = vectorInputDialogThreadSafe("Ввод плана X", "Введите компоненты вектора X:", n[k - 1])
                            ...
                            if (k > 1 && l > 1) {
                                ...
                                for (i in 0 until m[k - 1]) {
                                    ...
                                    db[i] = inputDialogThreadSafe("Ввод deltaB", "dB[${i + 1}]=").toDouble()
                                }
                            }
                            ...
                            if (l == 1) {
                                ...
                                val matrixA = matrixInputDialogThreadSafe("Ввод матрицы A", m[k - 1], n[k - 1])
    ...
    // Безопасная версия для потоков
    private fun inputDialogThreadSafe(title: String, message: String): String {
        val result = AtomicReference<String>("")
        val latch = CountDownLatch(1)
        
        Platform.runLater {
            try {
                val dialogResult = inputDialog(title, message)
    ...
    private fun vectorInputDialogThreadSafe(title: String, message: String, size: Int): DoubleArray {
        val result = AtomicReference<DoubleArray>(DoubleArray(size))
        val latch = CountDownLatch(1)
        
        Platform.runLater {
            try {
                val dialogResult = vectorInputDialog(title, message, size)
                result.set(dialogResult)
            } finally {
                latch.countDown()
            }
    ...
    private fun matrixInputDialogThreadSafe(title: String, rows: Int, cols: Int): Array<DoubleArray> {
        val result = AtomicReference<Array<DoubleArray>>(Array(rows) { DoubleArray(cols) })
        val latch = CountDownLatch(1)
        
        Platform.runLater {
            try {
                val dialogResult = matrixInputDialog(title, rows, cols)
                result.set(dialogResult)
            } finally {
                latch.countDown()
            }
